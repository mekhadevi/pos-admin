...
// DO NOT drop this table on startup — it must persist between
// restarts so the deduplication filter works correctly.
db.exec(`
  ...
  checkLowStock() {
        ...
        WHERE  p.IsDeleted        = 0
          ...
          AND  NOT EXISTS (
            ...
            WHERE  nl.Type        = 'stock'
              ...
              AND  nl.SentAt      = DATE('now', 'localtime')
          )
      `,
      )
      .all();
    ...
    for (const item of items) {
      ...
      const result = db
        .prepare(
          `
          INSERT OR IGNORE INTO NotificationLog (Type, ReferenceId, SentAt)
          VALUES ('stock', ?, DATE('now', 'localtime'))
        `,
        )
        .run(String(item.Id));
    ...
    for (const item of items) {
      ...
      const result = db
        .prepare(
          `
          INSERT OR IGNORE INTO NotificationLog (Type, ReferenceId, SentAt)
          VALUES ('expiry', ?, DATE('now', 'localtime'))
        `,
        )
        .run(String(item.Id));
    ...
    for (const reg of rows) {
      ...
      const result = db
        .prepare(
          `
          INSERT OR IGNORE INTO NotificationLog (Type, ReferenceId, SentAt)
          VALUES ('cashier', ?, DATE('now', 'localtime'))
        `,
        )
